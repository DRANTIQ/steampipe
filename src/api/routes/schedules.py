...
from src.api.deps import AuthUser, SuperAdmin, assert_tenant_access, resolve_list_tenant_filter
...
def create_scan_schedule(session: DbSession, body: ScheduleScanCreate, auth: SuperAdmin) -> QuerySchedule:
...
def create_schedule(session: DbSession, body: ScheduleCreate, auth: SuperAdmin) -> QuerySchedule:
...
def list_schedules(
    ...
    if scoped_tenant:
        q = q.filter(QuerySchedule.tenant_id == scoped_tenant)
